(function() {
  var destination, docco_styles, docco_template, ensure_directory, exec, ext, fs, generate_documentation, generate_html, get_language, highlight, highlight_end, highlight_start, l, languages, parse, path, showdown, spawn, template, _ref;
  ...
  generate_documentation = function(source, callback) {
    return fs.readFile(source, "utf-8", function(error, code) {
      ...
      return highlight(source, sections, function() {
        return callback(generate_html(source, sections));
      });
    });
  };
